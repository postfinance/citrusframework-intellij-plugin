...
public final class VirtualFileUtil {
	...
	public static String retrieveTestFileNames(VirtualFile[] virtualFiles) {
		...
		iterateTroughChildren(virtualFiles, (VirtualFile virtualFile) -> foundFiles.put(virtualFile.getPath(), virtualFile));

		return foundFiles.values().stream()
	...
	private static boolean isTestFile(VirtualFile virtualFile) {
		...
		return defaultExtension.equals(DEFAULT_EXTENSION) && virtualFile.getName().endsWith(TEST_XML);
	}
	 ...
	 */
	private static void iterateTroughChildren(VirtualFile[] virtualFiles, Consumer<VirtualFile> callback) {
